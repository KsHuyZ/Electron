 ...
 * @param contens
 */
export const removeAtPageStyle = async (
  webContents: WebContents
): Promise<String> => {
  const headHtml: any = await webContents.executeJavaScript(
    "document.head.innerHTML"
  );
  let newHeadHtml = headHtml.replaceAll(
    /@page\s*{[\s\S^}]*?margin[\s\S^}]*?}/g,
    ""
  );
...
export const isStyleRendered = async (webContens: WebContents) => {
  await webContens.executeJavaScript(
    `document.querySelectorAll('link[rel="stylesheet"]').length==[].slice.call(document.styleSheets).filter(({href})=>href!=null).length`
  );
  return true;
};
...
export const print_page = path.resolve(
