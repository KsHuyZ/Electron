...
db.run(
  "CREATE TABLE IF NOT EXISTS Delivery_Item (ID INTEGER PRIMARY KEY AUTOINCREMENT, id_Cout_Delivery INTEGER NOT NULL, name VARCHAR(100) NOT NULL, quantity INTEGER,quality INTEGER,price INTEGER,id_Warehouse INTEGER NOT NULL, FOREIGN KEY (id_Cout_Delivery) REFERENCES CountDelivery(ID), FOREIGN KEY (id_Warehouse) REFERENCES WareHouse(ID))"
);
